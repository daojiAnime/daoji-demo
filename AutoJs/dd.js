const SERVER_CHAN = 'SCT182314T0lfQ2nXln5jdqbBnWGeKHsqc';
const PACKAGE_ID_DD = 'com.alibaba.android.rimet'; // 钉钉
const PACKAGE_ID_WX = 'com.tencent.mm';
 ...
 */
  const access_token = '295a61a4333aa54ceb9ddc9b55bc20b12eeb0fc42cf000a874f392f2437b2d6d';
  const secret = 'SECc150ced6ca6661fdfe3b22f6e1bdb9be7bb82041e942cc9366fab573c5de7a29';
  const timestamp = Date.now().toString();
  const sign = hmac256(secret, timestamp);

  const url = `https://oapi.dingtalk.com/robot/send?access_token=${access_token}&timestamp=${timestamp}&sign=${sign}`;

  log(url);
  http.postJson(url, { msgtype: 'text', text: { content: msg } });
}
// 通知过滤器
function filterNotification(bundleId, abstract, text) {
  var check = PACKAGE_ID_WHITE_LIST.some(function (item) {
...
function hmac256(secret, timestamp) {
  // 创建Base64编码器
  var encoder = java.util.Base64.getEncoder();
  const stringToSign = timestamp + '\n' + secret;
  const mac = Mac.getInstance('HmacSHA256');
  mac.init(new SecretKeySpec(java.lang.String(secret).getBytes('UTF-8'), 'HmacSHA256'));
  const signData = mac.doFinal(java.lang.String(stringToSign).getBytes('UTF-8'));
  const sign = URLEncoder.encode(encoder.encodeToString(signData), 'UTF-8');
  log(sign);
  return sign;
}